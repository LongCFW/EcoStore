...
import { Container, Row, Col, Button, Carousel, Card, Badge, Spinner } from "react-bootstrap";
...
const HomePage = () => {
      ...
      try {
        ...
        if (prodRes && prodRes.data) {
            ...
            const mappedProducts = list.map(p => ({
                ...
                price: p.price_cents,
                salePrice: (p.compareAtPriceCents && p.compareAtPriceCents > p.price_cents) ? p.compareAtPriceCents : null, 
                // Lấy ảnh đầu tiên, fallback nếu không có
                image: p.images?.[0]?.imageUrl || 'https://placehold.co/300x300?text=No+Image',
        ...
        if (catRes) {
            ...
            else if (Array.isArray(catRes.categories)) catList = catRes.categories;
  ...
  const blogs = [
    ...
    { id: 2, title: "Lợi ích tuyệt vời của rau hữu cơ", img: "https://images.unsplash.com/photo-1540420773420-3366772f4999?q=80&w=1000&auto=format&fit=crop", date: "18/01/2025" },
  ...
  if (loading) {
      return <div className="d-flex justify-content-center align-items-center vh-100 bg-white"><Spinner animation="border" variant="success"/></div>;
  }
  ...
  return (
    <>
                ...
                <div className="hero-slide" style={{backgroundImage: 'url("https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=1500&q=80")'}}>
                    <div className="hero-overlay">
                        <div className="hero-content animate-slide-up">
                            <Badge bg="warning" text="dark" className="mb-3 px-3 py-2 rounded-pill">100% Organic</Badge>
                            <h1 className="display-4 fw-bold mb-3">Thực Phẩm Xanh <br/> Cho Cuộc Sống Lành</h1>
                            <p className="lead mb-4 opacity-75">Tươi ngon từ nông trại đến bàn ăn. Giảm thiểu rác thải.</p>
                            <Button as={Link} to="/products" variant="success" size="lg" className="rounded-pill px-5 shadow fw-bold border-0">Mua Ngay <FaArrowRight className="ms-2"/></Button>
                        </div>
                    </div>
                </div>
            </Carousel.Item>
                ...
                <div className="hero-slide" style={{backgroundImage: 'url("https://images.unsplash.com/photo-1605647540924-852290f6b0d5?auto=format&fit=crop&w=1500&q=80")'}}>
                    <div className="hero-overlay">
                        <div className="hero-content">
                            ...
                            <p className="lead mb-4 opacity-75">Bộ sưu tập sản phẩm thân thiện với mẹ thiên nhiên.</p>
                            <Button as={Link} to="/products" variant="light" size="lg" className="rounded-pill px-5 shadow fw-bold text-success border-0">Khám Phá <FaArrowRight className="ms-2"/></Button>
                        </div>
                    </div>
                </div>
            </Carousel.Item>
                            ...
                            <Link to={`/products?category=${cat._id}`} className="text-decoration-none">
                                <div className="category-card">
                                    <div className="cat-img-wrapper">
                                        <img src={getCategoryImage(cat, idx)} alt={cat.name} onError={(e) => {e.target.src = 'https://placehold.co/150?text=Eco'}} />
                                    </div>
                                    <h6 className="fw-bold text-dark mt-3 text-truncate px-2" title={cat.name}>{cat.name}</h6>
                                </div>
                            </Link>
                        </Col>
                    ))}
        ...
        <Container>
            <div className="d-flex justify-content-between align-items-end mb-4">
                <div>
                    <div className="d-flex align-items-center gap-2 mb-1">
                        ...
                        <Badge bg="danger" className="animate-pulse">New</Badge>
                    </div>
                    <p className="text-muted mb-0">Những món hàng xanh vừa cập bến</p>
                </div>
                <Button as={Link} to="/products" variant="outline-success" className="rounded-pill fw-bold">Xem tất cả</Button>
            </div>
            ...
            {products.length > 0 ? (
                ...
                <div className="text-center py-5 text-muted">
                    ...
                    <small>Vui lòng vào Admin thêm sản phẩm hoặc kiểm tra kết nối DB.</small>
                </div>
            )}
        </Container>
      </section>
      ...
      <section className="mb-5">
        <Container>
            <div className="static-banner rounded-4 overflow-hidden position-relative shadow-sm" style={{height: '300px'}}>
                <img 
                ...
                <div className="position-absolute top-0 start-0 w-100 h-100 bg-dark bg-opacity-50 d-flex align-items-center justify-content-center text-center px-3">
                    <div className="text-white" style={{maxWidth: '700px'}}>
                        ...
                        <p className="fs-5 mb-4 opacity-90">Khám phá bộ sưu tập nước ép trái cây và rau củ hữu cơ giải nhiệt mùa hè.</p>
                        <Button as={Link} to="/products" variant="light" size="lg" className="rounded-pill px-5 fw-bold text-success">Mua Ngay</Button>
                    </div>
                </div>
            </div>
        </Container>
          ...
          <section className="mb-5">
            <Container>
                <div className="text-center mb-5">
                    ...
                    <p className="text-muted w-75 mx-auto">Các sản phẩm được yêu thích nhất tại EcoStore</p>
                </div>
                <Row xs={1} md={2} lg={4} className="g-4">
                ...
                <div className="text-center mt-5">
                    <Button as={Link} to="/products" variant="outline-success" size="lg" className="rounded-pill px-5">Xem thêm sản phẩm</Button>
                </div>
            </Container>
          </section>
      )}
                    ...
                    <Col md={4} key={blog.id}>
                        <Card className="blog-card h-100">
                            ...
                            <Card.Body>
                                ...
                                <Card.Title className="fw-bold mb-2 fs-5 hover-green" style={{cursor:'pointer'}}>{blog.title}</Card.Title>
