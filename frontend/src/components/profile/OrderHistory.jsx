            ...
            <Modal.Body>
                        ...
                        <tbody>
                            {selectedOrder?.items?.map((item, idx) => (
                                <tr key={idx} className="border-bottom">
                                    <td style={{width: '60px'}}>
                                        <img src={item.image || 'https://placehold.co/50'} alt="" width="50" height="50" className="rounded object-fit-cover border"/>
                                    </td>
                                    <td>
                                    ...
                                    <td className="text-end fw-bold">{(item.price_cents * item.quantity).toLocaleString()} đ</td>
                                </tr>
                            ))}
                        </tbody>
                    </Table>
                ...
                {selectedOrder?.note && (
                    <div className="mt-3 p-2 bg-light rounded border border-warning border-opacity-25">
                        <small className="text-muted fw-bold">Ghi chú:</small> <span className="small">{selectedOrder.note}</span>
                    </div>
                )}

                <h5 className="text-end text-success mt-3 pt-2 border-top">Tổng tiền: {selectedOrder?.totalAmount_cents?.toLocaleString()} đ</h5>
            </Modal.Body>
            <Modal.Footer>
